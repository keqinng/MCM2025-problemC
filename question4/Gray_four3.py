...
def GRA_multi_factors(data, reference_col='W'):
    """
    多因素灰色关联分析
...
try:
    ...
    df = pd.read_excel('cleaneddata.xlsx', 
                      ...
                      engine='openpyxl',
                      header=None,
                      usecols=[4,5,7,12,13,17,22],  # 扩展读取更多列
                      skiprows=1,
                      nrows=66)
    
    # 重命名列
    df.columns = [ 'C','D', 'H', 'K', 'N','P','W']  # 根据实际数据调整列名
    ...
    # 执行多因素灰色关联分析
    results = GRA_multi_factors(df, reference_col='W')
    ...
    for factor, gra_value in results.items():
        ...
        print(f'{factor_name}和13号染色体是否异常的关联: {str(gra_value)}')
...
def visualize_gra_results_bar(results_dict):
    ...
    # 设置标题和标签
    plt.title('各因素与13号染色体异常的灰色关联度', pad=20, fontsize=14)
...
# 示例数据（使用你提供的结果）
results = {
    'BMI': pd.DataFrame([[1.000000, 0.604844], [0.604844, 1.0]]),
    '原始读段数': pd.DataFrame([[1.00000, 0.589006], [0.589006, 1.0]]),
    '重复读段比例': pd.DataFrame([[1.000000, 0.625902], [0.625902, 1.0]]),
    '13号染色体Z值': pd.DataFrame([[1.000000, 0.581919], [0.581919, 1.0]]),
    'X染色体Z值': pd.DataFrame([[1.000000, 0.766668], [0.766668, 1.0]]),
    '13号染色体GC含量': pd.DataFrame([[1.000000, 0.65648], [0.65648, 1.0]])
}
