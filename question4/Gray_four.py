...
def GRA_multi_factors(data, reference_col='U'):
    """
    多因素灰色关联分析
...
try:
    ...
    df = pd.read_excel('cleaneddata.xlsx', 
                      ...
                      engine='openpyxl',
                      header=None,
                      usecols=[4,5,7,12,13,17,20],  # 扩展读取更多列
                      skiprows=1,
                      nrows=66)
    
    # 重命名列
    df.columns = [ 'C','D', 'H', 'M', 'N','R','U']  # 根据实际数据调整列名
    ...
    # 执行多因素灰色关联分析
    results = GRA_multi_factors(df, reference_col='U')
    ...
    for factor, gra_value in results.items():
        ...
        print(f'{factor_name}和21号染色体是否异常的关联: {str(gra_value)}')
    

    
except FileNotFoundError:
    print("错误：找不到文件 'cleaneddata.xlsx'")
...
def visualize_gra_results_bar(results_dict):
    ...
    # 设置标题和标签
    plt.title('各因素与21号染色体异常的灰色关联度', pad=20, fontsize=14)
...
# 示例数据（使用你提供的结果）
results = {
    'BMI': pd.DataFrame([[1.000000, 0.672418], [0.672418, 1.0]]),
    '原始读段数': pd.DataFrame([[1.00000, 0.710025], [0.710025, 1.0]]),
    '重复读段比例': pd.DataFrame([[1.000000, 0.713538], [0.713538, 1.0]]),
    '21号染色体Z值': pd.DataFrame([[1.000000, 0.69079], [0.69079, 1.0]]),
    'X染色体Z值': pd.DataFrame([[1.000000, 0.700442], [0.700442, 1.0]]),
    '21号染色体GC含量': pd.DataFrame([[1.000000, 0.755684], [0.755684, 1.0]])
}
