...
# 对其他变量也进行分组
data['年龄_quartile'] = pd.qcut(data['年龄'], q=2, labels=['年轻',  '中年'])
data['BMI_quartile'] = pd.qcut(data['BMI'], q=4, labels=['Q1', 'Q2', 'Q3', 'Q4'])

# 创建多变量交叉分组
data['多因素分组'] = data['BMI_quartile'].astype(str) + "_" + data['年龄_quartile'].astype(str)
# 创建综合评分
from sklearn.preprocessing import StandardScaler

# 标准化各变量
scaler = StandardScaler()
data_scaled = pd.DataFrame(scaler.fit_transform(data[['BMI', '年龄']]), 
                           columns=['BMI_scaled', '年龄_scaled'])

# 计算综合评分（可根据实际情况调整权重）
data['综合评分'] = 0.7 * data_scaled['BMI_scaled'] + 0.3 * data_scaled['年龄_scaled'] 
...
# 绘制箱线图，展示各分组的Y染色体浓度分布
plt.figure(figsize=(12, 6))
sns.boxplot(x='多因素分组', y='Y染色体浓度', data=data)
